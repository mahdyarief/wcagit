...
export class A11yEngine {
  ...
  async analyzeUrl(url: string, options: AnalysisOptions = {}) {
  ...
  private async performAAAChecks(page: any) {
  ...
  async analyzeHtml(html: string, options: AnalysisOptions = {}) {
  ...
  private formatResults(result: AxeResults) {
    return {
      ...
      incomplete: result.incomplete.map((inc: Result) => ({
        id: inc.id,
        impact: inc.impact || 'unknown',
        description: inc.description,
        help: inc.help,
        helpUrl: inc.helpUrl,
        affectedNodes: inc.nodes.map((node: NodeResult) => ({
          html: node.html,
          target: node.target,
          failureSummary: node.failureSummary || ''
        }))
  ...
  calculateContrastRatio(color1: { r: number, g: number, b: number }, color2: { r: number, g: number, b: number }): number {
