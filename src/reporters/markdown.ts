export function formatToMarkdown(results: any): string {
  ...
  if (results.violations.length > 0) {
    ...
    results.violations.forEach((violation: any) => {
      ...
      md += `#### Affected Nodes\n`;
      violation.affectedNodes.forEach((node: any) => {
        md += `- \`${node.html}\`\n`;
  ...
  if (results.incomplete && results.incomplete.length > 0) {
    ...
    results.incomplete.forEach((inc: any) => {
      md += `### ${inc.id}\n`;
      md += `**Description:** ${inc.description}\n`;
      md += `**Help:** [${inc.help}](${inc.helpUrl})\n\n`;
      
      md += `#### Nodes to Review\n`;
      inc.affectedNodes.forEach((node: any) => {
        md += `- \`${node.html}\`\n`;
        md += `  - Summary: ${node.failureSummary}\n`;
      });
      md += `\n`;
  ...
  if (results.aaaInsights) {
    ...
    if (results.aaaInsights.targetSizeViolations.length > 0) {
      ...
      results.aaaInsights.targetSizeViolations.forEach((v: any) => {
  ...
  const hasImages = results.violations.some((v: any) => v.id.includes('image') || v.id.includes('alt')) || 
                    results.incomplete.some((v: any) => v.id.includes('image') || v.id.includes('alt'));
  if (hasImages) {
    md += `- [ ] **Image Alt Text:** Review images to ensure alt text is meaningful and not redundant.\n`;
  }

  const hasVideo = results.url.includes('video') || results.incomplete.some((v: any) => v.id.includes('video'));
  if (hasVideo) {
